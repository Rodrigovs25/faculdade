...
int main()
{
    ...
    // Cálculo do número total de nós no grafo (2 por célula + fonte(origem) + sumidouro(destino))
    int totalDeNos = n * m * 2 + 2;
    int origem = totalDeNos - 2;
    int destino = totalDeNos - 1;

    FluxoMaximo fluxo(totalDeNos);
    fluxo.setLimite(somaTotal); // Capacidade Máxima
