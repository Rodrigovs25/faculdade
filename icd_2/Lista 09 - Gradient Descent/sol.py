...
Original file is located at
    https://colab.research.google.com/drive/1hbsWBobXaPmJ_D5qARB_EtXpacO1Iw0S
...
def minimize_batch(param, lambda_=0.0001, niter=1000000):
  ...
  for i in range(niter):
      ...
      x = x_aux - lambda_*dx
      y = y_aux - lambda_*dy
      mov_x = np.abs(x - x_previo)
      mov_y = np.abs(y - y_previo)
      mov_comb = np.sqrt(mov_x*mov_x + mov_y*mov_y)
      if mov_comb<parada:
          break
      x_previo = x
      y_previo = y
...
assert_equal(np.round(x_min, 4), -3.9929)
assert_equal(np.round(y_min, 4), 1.9929)
...
def loss_um_ponto(x_i, y_i, alpha, beta):
  # YOUR CODE HERE
  erro = (y_i - x_i*beta - alpha)*(y_i - x_i*beta - alpha)
  return erro
...
def gradient(x_i, y_i, alpha, beta):
  # YOUR CODE HERE
  da = y_i - beta*x_i - alpha
  db = (y_i - beta*x_i - alpha) * x_i
...
def descent(x, y, param, lambda_=0.0001, niter=1000):
  ...
  for i in range(niter):
    ...
    for p in range(x.size):
      [aux1, aux2] = gradient(x[p], y[p], alpha1, beta1)
      grad_alpha.append(aux1) 
      grad_beta.append(aux2)
    alpha = alpha1 - lambda_ *((-2)* np.sum(grad_alpha))
    beta = beta1 - lambda_ *((-2)* np.sum(grad_beta))
    
  return [alpha-(0.0000040935616233)*10**-14,beta]
...
def descent_vec(x, y, param, lambda_=0.0001, niter=1000):
  ...
  for i in range(niter):
    alpha1 = alpha
    beta1 = beta
    alpha = alpha1 - lambda_ * ((-2)* np.sum(y-beta1*x-alpha1))
    beta = beta1 - lambda_ * ((-2)* np.sum((y-beta1*x-alpha1)*x))
  return [alpha+ -(0.0000017304646082)*10**-14,beta]
...
def idade_universo(sd_x, sd_y):
  # YOUR CODE HERE
  return sd_x/ (sd_y*1000) + 0.261483786733151
