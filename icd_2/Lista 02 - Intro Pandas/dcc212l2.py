...
Original file is located at
    https://colab.research.google.com/github/icd-ufmg/icd-ufmg.github.io/blob/master/listas/l2/dcc212l2.ipynb
...
def month_sales(df, month: str):
    x = pd.Series(data=df.loc[month], index=['icecream', 'sunglasses', 'coats'])
    return x
...
def row_sales(df, row: int):
    y = pd.Series(data=df.iloc[row], index=['icecream', 'sunglasses', 'coats'])
    return y
...
def questao6(df):
    stats = pd.DataFrame(df, columns=['icecream', 'sunglasses', 'coats'])
...
def mortes_por_pump(df):
  y = df.groupby("NearestPumpID")["Count"].sum()
  return y