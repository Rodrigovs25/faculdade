...
Original file is located at
    https://colab.research.google.com/drive/1Hc0LFtbkq2vqCBhhq7EayWBBiuJDJCKZ
...
def sgd(theta, d_fun, X, y, lambda_=0.001, tol=0.01, max_iter=10000):
  ...
  for i in range(max_iter):
    ...
    if err_sq < tol:
      ...
      #print('Iter {}; alpha, beta = '.format(i+1), theta)  return theta
...
X.insert(0,'intercepto',np.ones(y.shape))
...
def descent(theta0, X, y, learning_rate=0.005, tolerance=0.0000001):
  ...
  old_err_sq = np.inf
  while True:
    theta_novo = theta - learning_rate * gradients(theta, X, y)
    err_sq = ((X.dot(theta)-y)**2).mean()
