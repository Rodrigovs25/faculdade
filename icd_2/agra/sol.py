...
# Defina uma função para calcular o intervalo de confiança
def CI(data):
    # Filtrar o DataFrame para selecionar apenas as escolas com 'DEPENDENCIA_ADMINISTRATIVA' igual a 'Estadual'
    escolas_estaduais = data[data['DEPENDENCIA_ADMINISTRATIVA'] == 'Estadual']

    # Calcular a média e o erro padrão da variável 'NOTA_MEDIA_ESCOLA' para as escolas Estaduais
    media = escolas_estaduais['NOTA_MEDIA_ESCOLA'].mean()
    erro_padrao = escolas_estaduais['NOTA_MEDIA_ESCOLA'].std() / np.sqrt(len(escolas_estaduais))

    # Definir o nível de confiança (99%)
    alpha = 0.01

    # Calcular o valor crítico a partir da distribuição t para um intervalo de confiança de 99%
    t_critico = ss.t.ppf(1 - alpha / 2, df=len(escolas_estaduais) - 1)
...
# Selecione a coluna 'TAXA_DE_APROVACAO'
taxa_aprovacao = data['TAXA_DE_APROVACAO']

# Crie um objeto ECDF
ecdf = ECDF(taxa_aprovacao)

# Plote a CDF sem linhas de marcação, mas com os números
plt.step(ecdf.x, ecdf.y, where='mid')
plt.xlabel('Taxa de Aprovação')
plt.ylabel('CDF')
plt.title('CDF da Taxa de Aprovação')
plt.grid(False)  # Desativa as linhas de marcação
plt.show()
...
def bootstrap_mean(n_sub, alpha):
    ...
    federal_data = data[data['DEPENDENCIA_ADMINISTRATIVA'] == 'Federal']
    size = len(federal_data)
    values = np.zeros(n_sub)

    for i in range(n_sub):
        sample = federal_data.sample(size, replace=True)
        values[i] = sample[col].mean()

    LI = np.percentile(values, (1 - alpha) * 100 / 2)
    LS = np.percentile(values, 100 - (1 - alpha) * 100 / 2)
...
def ci_amplitude():
    n_sub = 5000
    alpha = 0.95
    amplitudes = {}  # Um dicionário para armazenar as amplitudes para cada categoria

    for dependency in data['DEPENDENCIA_ADMINISTRATIVA'].unique():
        subset = data[data['DEPENDENCIA_ADMINISTRATIVA'] == dependency]
        values = np.zeros(n_sub)

        for i in range(n_sub):
            sample = subset.sample(len(subset), replace=True)
            values[i] = np.percentile(sample['TAXA_DE_PARTICIPACAO'], 50)

        LI = np.percentile(values, (1 - alpha) * 100 / 2)
        LS = np.percentile(values, 100 - (1 - alpha) * 100 / 2)
        amplitude = LS - LI
        amplitudes[dependency] = amplitude

    # Encontre a categoria com a maior amplitude
    max_amplitude_category = max(amplitudes, key=amplitudes.get)
    max_amplitude_value = amplitudes[max_amplitude_category]

    return (max_amplitude_category, max_amplitude_value)
