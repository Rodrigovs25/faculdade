...
def percent(df1, df2, alpha):
    # Realiza o teste de Mann-Whitney U para comparar as medianas
    statistic, p_value = mannwhitneyu(df1['TAXA_DE_PARTICIPACAO'], df2['TAXA_DE_PARTICIPACAO'], alternative='two-sided')

    # Compara o valor-p com o nível de significância (α)
    if p_value < alpha:
        return True  # Há diferença significativa
    else:
        return False  # Não há diferença significativa
...
def resposta(data):
  ...
  for i in range(5000):
    data_tmp = data.copy()
    times = list(data_tmp['TEAM'].values)
    np.random.shuffle(times)
    data_tmp['SHUFFLED_TEAM'] = times
  ...
  if(t_obs(data) >= np.percentile(diferencas, 5) and t_obs(data) <= np.percentile(diferencas, 95)):
    return True
  else:
    return False
...
def t_obs2(data):
    return data['player_height'][data['team_abbreviation'] == 'CLE'].mean() - data['player_height'][data['team_abbreviation'] == 'GSW'].mean()
...
def resposta2(diffs, t_obs):
    ...
    critico_inferior, critico_superior = critical_values(diffs)
    if t_obs < critico_inferior or t_obs > critico_superior:
        return True
    else:
        return False
